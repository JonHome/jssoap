		...
		template : template(function(){/*
			<div  class="content">
				...
				<form class="input-group">
					<div class="input-row">
						<label>数量</label>
						<input type="text" placeholder="输入数量" value="1" style="text-align:right">
					</div>
				</form>
				<div style="padding:10px;">
					<button class="btn btn-negative btn-block" v-if="!pending1" @click="remove">删除材料</button>
					<button disabled class="btn btn-negative btn-block" v-if="pending1"><i class="fa fa-spinner fa-spin"></i></button>
				</div>
				<div style="padding:10px 10px 0;background:#fff;position:fixed;bottom:0;width:100%;">
					<button class="btn btn-positive  btn-block" v-if="!pending" @click="next">保存修改</button>
